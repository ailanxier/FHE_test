...
void client_init_context(char* path, CryptoContext<DCRTPoly>& client_context, std::fstream& info_fstream){
    ...
    Print_words({"muldepth = ", ToStr(muldepth)}, 2);
...
void server_init_context(CryptoContext<DCRTPoly> &server_context, std::fstream& info_fstream, std::fstream& result_fstream){
